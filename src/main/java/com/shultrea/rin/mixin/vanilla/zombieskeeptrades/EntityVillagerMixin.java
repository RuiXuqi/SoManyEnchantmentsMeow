...
public abstract class EntityVillagerMixin extends EntityLivingBase implements IEntityVillagerMixin {
    ...
    @Override
    public void soManyEnchantments$setTradesFromNBT(NBTTagCompound compound) {
        ...
        if(compound.hasKey("Offers", 10)) {
            ...
            for(MerchantRecipe recipe : this.buyingList){
                if(this.getRNG().nextFloat() < 0.2F) ((IMerchantRecipeMixin) recipe).soManyEnchantments$increasePrices();
                if(this.getRNG().nextFloat() < 0.2F) ((IMerchantRecipeMixin) recipe).soManyEnchantments$denyXP();
            }
        }
    }
}