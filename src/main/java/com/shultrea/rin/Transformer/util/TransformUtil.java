...
public class TransformUtil {
	...
	@Nullable
	/** REQUIRES MAXS : Creates, but does not add, a new local variable node */ public static LocalVariableNode createNewLocalVariable(final MethodNode methodNode, String name, String desc) {
		int index = 0;
		for(LocalVariableNode lvn : methodNode.localVariables) {
			if(lvn.index >= index) index = lvn.index + 1;
		}
