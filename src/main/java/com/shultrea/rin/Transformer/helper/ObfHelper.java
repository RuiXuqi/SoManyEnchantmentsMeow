...
public class ObfHelper {
	...
	/**
	 * Should be initialized to true by a core mod that uses SortingIndex > 1000 (e.g. in
	 * {@link net.minecraftforge.fml.relauncher.IFMLLoadingPlugin#injectData}).
	 */
	 ...
	 * using the value of "runtimeDeobfuscationEnabled" to avoid the class loader lookup in isObfuscated.<br>
	 * <br>
	 * <b>Example:</b>
	 * <pre>
	...
	public static String toDeobfClassName(String obfClassName) {
		if(isObfuscated() && !runsAfterDeobfRemapper()) return forceToDeobfClassName(obfClassName);
		else return obfClassName;
	}
	...
	public static String toObfClassName(String deobfClassName) {
		if(isObfuscated() && !runsAfterDeobfRemapper()) return forceToObfClassName(deobfClassName);
		else return deobfClassName;
	}
