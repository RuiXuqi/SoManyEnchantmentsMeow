package com.shultrea.rin.Transformer.helper;
...
public class ASMHelper {
	...
	/**
	 * Turns the given return and parameter values into a method descriptor Converts the types into descriptors as
	 * needed
	 * @return (LparamType ;)LreturnType;
	 */
	public static String toMethodDescriptor(String returnType, String... paramTypes) {
		StringBuilder paramDescriptors = new StringBuilder();
		for(String paramType : paramTypes) {paramDescriptors.append(toDescriptor(paramType));}
		return "(" + paramDescriptors + ")" + toDescriptor(returnType);
	}
	...
	/**
	 * Convert a ClassNode into a byte array. Attempts to resolve issues with resolving super classes in an obfuscated
	 * environment. See {@link ObfRemappingClassWriter}.
	 */
	...
	public static byte[] writeClassToBytes(ClassNode classNode, int flags) {
		...
		else return writeClassToBytesNoDeobf(classNode, flags);
	}
	
	/**
	 * Convert a ClassNode into a byte array. Will have issues with resolving super classes in an obfuscated
	 * environment.
	 */
	...
	private static Collection<String> findAllInterfaces(ClassReader classReader) {
		...
		try {
			if(classHasSuper(classReader)) {
				...
				else interfaces.addAll(findAllInterfaces(getClassReaderForClassName(className2)));
			}
		}
		catch(IOException e) {
	...
	public static boolean doesClassImplement(ClassReader classReader, String targetInterfaceInternalClassName) {
		if(!INTERFACE_LOOKUP_CACHE.containsKey(classReader.getClassName())) findAllInterfaces(classReader);
	...
	public static boolean doesClassExtend(ClassReader classReader, String targetSuperInternalClassName) {
		if(!classHasSuper(classReader)) return false;
		String immediateSuperName = ObfHelper.getInternalClassName(classReader.getSuperName());
		if(immediateSuperName.equals(targetSuperInternalClassName)) return true;
		try {
	...
	/**
	 * @return The first instruction for which {@link AbstractInsnNode#getType()} == {@code type} (could be
	 * {@code firstInsnToCheck}). If a matching instruction cannot be found, returns {@code null}.
	 */
	...
	/**
	 * @return The first instruction for which {@link AbstractInsnNode#getType()} == {@code type} (could be
	 * {@code firstInsnToCheck}). If {@code reverseDirection} is {@code true}, instructions will be traversed backwards
	 * (using getPrevious()). If a matching instruction cannot be found, returns {@code null}.
	 */
	public static AbstractInsnNode getOrFindInstructionOfType(AbstractInsnNode firstInsnToCheck, int type, boolean reverseDirection) {
		for(
				AbstractInsnNode instruction = firstInsnToCheck; instruction != null; instruction =
				reverseDirection ? instruction.getPrevious() : instruction.getNext()) {
			if(instruction.getType() == type) return instruction;
		}
		return null;
	}
	
	/**
	 * @return The first instruction for which {@link AbstractInsnNode#getOpcode()} == {@code opcode} (could be
	 * {@code firstInsnToCheck}). If a matching instruction cannot be found, returns {@code null}
	 */
	...
	/**
	 * @return The first instruction for which {@link #isLabelOrLineNumber} == {@code true} (could be
	 * {@code firstInsnToCheck}). If {@code reverseDirection} is {@code true}, instructions will be traversed backwards
	 * (using getPrevious()). If a matching instruction cannot be found, returns {@code null}.
	 */
	...
	public static AbstractInsnNode getOrFindLabelOrLineNumber(AbstractInsnNode firstInsnToCheck, boolean reverseDirection) {
		for(
				AbstractInsnNode instruction = firstInsnToCheck; instruction != null; instruction =
				reverseDirection ? instruction.getPrevious() : instruction.getNext()) {
			if(isLabelOrLineNumber(instruction)) return instruction;
		}
		return null;
	}
	
	/**
	 * @return The first instruction for which {@link #isLabelOrLineNumber} == {@code false} (could be
	 * {@code firstInsnToCheck}). If a matching instruction cannot be found, returns {@code null}.
	 */
	...
	/**
	 * @return The first instruction of the {@code method} for which {@link #isLabelOrLineNumber} == {@code false}. If a
	 * matching instruction cannot be found, returns {@code null}.
	 */
	...
	/**
	 * @return The first instruction of the {@code method} for which {@link AbstractInsnNode#getOpcode()} ==
	 * {@code opcode}. If a matching instruction cannot be found, returns {@code null}.
	 */
	...
	/**
	 * @return The last instruction of the {@code method} for which {@link AbstractInsnNode#getOpcode()} ==
	 * {@code opcode}. If a matching instruction cannot be found, returns {@code null}.
	 */
	 ...
	 * (excluding {@code instruction}). If a matching instruction cannot be found, returns {@code null}.
	 */
	...
	/**
	 * @return The next instruction after {@code instruction} for which {@link AbstractInsnNode#getOpcode()} ==
	 * {@code opcode} (excluding {@code instruction}). If a matching instruction cannot be found, returns {@code null}.
	 */
	...
	/**
	 * @return The previous instruction before {@code instruction} for which {@link #isLabelOrLineNumber} ==
	 * {@code false} (excluding {@code instruction}). If a matching instruction cannot be found, returns {@code null}.
	 */
	...
	/**
	 * @return The previous instruction before {@code instruction} for which {@link AbstractInsnNode#getOpcode()} ==
	 * {@code opcode} (excluding {@code instruction}). If a matching instruction cannot be found, returns {@code null}.
	 */
	...
	/**
	 * @return The previous instruction before {@code instruction} for which {@link #isLabelOrLineNumber} ==
	 * {@code true} (excluding {@code instruction}). If a matching instruction cannot be found, returns {@code null}.
	 */
	...
	/**
	 * @return The method of the class that has both a matching {@code methodName} and {@code methodDesc}. If no
	...
	/**
	 * @return The method of the class that has a matching {@code srgMethodName} or {@code mcpMethodName} and a matching
	 * {@code methodDesc}. If no matching method is found, returns {@code null}.
	 */
	 ...
	 * @return The last label of the method (usually after the RETURN instruction). If no matching {@link LabelNode} is
	...
	/**
	 * Remove instructions from {@code insnList} starting with {@code startInclusive} up until reaching
	 * {@code endNotInclusive} ({@code endNotInclusive} will not be removed).
	 ...
	 * {@link #removeFromInsnListUntil(InsnList, AbstractInsnNode, AbstractInsnNode) removeFromInsnListUntil} and will
	 * achieve a similar result. <br>
	 * <br>
	 *
	 * Skip instructions from {@code insnList} starting with {@code startInclusive} up until reaching
	 * {@code endNotInclusive} ({@code endNotInclusive} will not be skipped).
	 *
	 * This is achieved by inserting a GOTO instruction before {@code startInclusive} which is branched to a LabelNode
	 * that is inserted before {@code endNotInclusive}.
	 */
	...
	/**
	 * Note: Does not move the instruction, but rather gets the instruction a certain distance away from
	 * {@code start}.<br>
	 * <br>
	 *
	 * <b>Example:</b>
	 ...
	 * @return All of the instructions that were matched by the {@code checkFor} pattern. If the pattern was not found,
	 * returns an empty {@link InsnList}.<br>
	 * <br>
	...
	public static InsnList checkForPatternAt(InsnList checkFor, AbstractInsnNode checkAgainst) {
		...
		for(AbstractInsnNode lookFor = checkFor.getFirst(); lookFor != null; ) {
			if(checkAgainst == null) return new InsnList();
			if(isLabelOrLineNumber(lookFor)) {
				lookFor = lookFor.getNext();
				continue;
			}
			if(isLabelOrLineNumber(checkAgainst)) {
				if(firstNeedleFound) foundInsnList.add(checkAgainst);
				checkAgainst = checkAgainst.getNext();
				continue;
			}
			if(!instructionsMatch(lookFor, checkAgainst)) return new InsnList();
	 ...
	 * @return All of the instructions that were matched by the pattern. If the pattern was not found, returns an empty
	 * {@link InsnList}.<br>
	 * <br>
	 * Note: If the pattern was matched, the size of the returned {@link InsnList} will be >= {@code checkFor}.size().
	 */
	public static InsnList findAndGetFoundInsnList(AbstractInsnNode haystackStart, InsnList needle) {
		...
		while(checkAgainstStart != null) {
			...
			if(found.getFirst() != null) return found;
	 ...
	 * @return The first instruction of the matched pattern. If the pattern was not found, returns an empty
	 * {@link InsnList}.
	 */
	...
	/**
	 * Searches for the pattern in {@code needle}, starting at {@code haystackStart}.
	 * @return The first instruction of the matched pattern. If the pattern was not found, returns {@code null}.
	 */
	public static AbstractInsnNode find(AbstractInsnNode haystackStart, InsnList needle) {
		if(needle.getFirst() == null) return null;
	 ...
	 * @return The matching instruction. If a matching instruction was not found, returns {@code null}.
	 */
	 ...
	 * @return The instruction after the replacement. If the pattern was not found, returns {@code null}.
	 */
	...
	/**
	 * Searches for the pattern in {@code needle} within {@code haystack} (starting at {@code haystackStart}) and
	...
	/**
	 * Searches for all instances of the pattern in {@code needle} within {@code haystack} and replaces them with
	 * {@code replacement}.
	...
	/**
	 * Searches for all instances of the pattern in {@code needle} within {@code haystack} (starting at
	 * {@code haystackStart}) and replaces them with {@code replacement}.
	 * @return The number of replacements made.
	 */
	...
	/**
	 * @return The local variable of the method that has both a matching {@code varName} and {@code varDesc}. If no
	 * matching local variable is found, returns {@code null}.
	 */
