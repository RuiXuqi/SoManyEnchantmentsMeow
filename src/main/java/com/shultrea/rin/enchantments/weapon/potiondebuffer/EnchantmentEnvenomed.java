...
public class EnchantmentEnvenomed extends EnchantmentBase {
	...
	public boolean canApplyAtEnchantingTable(ItemStack stack){
		return ConfigProvider.canItemApply(this, ModConfig.canApply.envenomed, stack) && super.canApplyAtEnchantingTable(stack);
	}

	@Override
	public boolean canApply(ItemStack stack){
		return ConfigProvider.canItemApply(this, ModConfig.canApplyAnvil.envenomed, stack) || super.canApply(stack);
	}
	...
	public void onEntityDamagedAlt(EntityLivingBase attacker, Entity target, ItemStack weapon, int level) {
		...
		if (attacker.getRNG().nextFloat() <= 0.2F * (float) level) {
            ...
            if (level > 2) victim.addPotionEffect(new PotionEffect(MobEffects.WITHER, 40 + (10 * level), level - 1));
        }
    }
}